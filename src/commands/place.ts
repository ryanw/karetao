...
export default class PlaceCommand extends Command {
  execute(robot: Robot, arg: string) {
    super.execute(robot, arg);
    ...
    if (Object.values(Direction).indexOf(args[2]) === -1) throw new Error(`${args[2]} is not a valid direction`);
    const direction: Direction = args[2]
